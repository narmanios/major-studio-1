    ...
    function showCarouselImages() {
      ...
      for (var i = carouselIndex; i < carouselIndex + visibleCount; i++) {
        if (carouselImages[i]) {
          ...
          // --- Add a container for the p5 canvas ---
          var canvasDiv = document.createElement("div");
          canvasDiv.id = "p5-canvas-" + j;
          canvasDiv.className = "p5-canvas-container";
          cell.appendChild(canvasDiv);

          frag.appendChild(cell);

          // --- Create a p5 sketch for this silhouette ---
          let points = getSilhouette(it); // get points for this item

          new p5(function (p) {
            p.setup = function () {
              p.createCanvas(120, 120).parent(canvasDiv);
              p.noLoop();

              p.background(255);

              // If you have an image to draw, load and draw it here
              // (You'd need to use p.loadImage and p.image if you want to draw the thumbnail)
              ...
              for (let i = 0; i < points.length; i++) {
                const pt = points[i];
                p.ellipse(pt.x / 4, pt.y / 4, 5, 5); // scale as in your example
              }
            };
          });
