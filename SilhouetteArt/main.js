...
d3.json("data/dataset_silhouettes_only_with_filename.json")
  .then((data) => {
    ...
    function showFullscreenChart(items) {
      ...
      function extractYearFromRecord(it) {
        if (!it) return null;
        var raw =
          it.date ?? it.dates ?? it.indexed_dates ?? it.objectDate ?? "";
      ...
      for (var i = 0; i < yearsArr.length; i++) {
        ...
        if (
          !yearBlocks.length ||
          yearBlocks[yearBlocks.length - 1].year !== yr
        ) {
  ...
  `;
    ...
    ready(function () {
      var chartsBtn = document.getElementById("charts");
      if (!chartsBtn) return;

      // charts click => show fullscreen chart overlay (uses existing showFullscreenChart)
      chartsBtn.addEventListener("click", function (e) {
        e.preventDefault();
        // compute itemsToShow as before
        var itemsToShow = [];
        try {
          if (typeof activeTopic !== "undefined" && activeTopic) {
            itemsToShow = filterByTopic(silhouettes || [], activeTopic);
          }
        } catch (err) {
          itemsToShow = [];
        }
        ...
        if (typeof showFullscreenChart === "function") {
          showFullscreenChart(itemsToShow);
        }
      });
    })();
  })
